...
# --- OLD CORE ALGORITHM (FIXED RANGE) ---
def solve_deployment(params):
    logger = OptimizationLogger()
    logger.log("--- INITIALIZING FIXED-RANGE MISSION ---")
    ...
    logger.log(f"Grid: {num_locations} sites | Targets: {num_targets} | Max Sensors: {params['max_sensors']}")
    ...
    # x[i] is 1 if a sensor is placed at location i
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        # Returns simple [x, y] coordinates for the old frontend
...
def optimize_api():
    try:
        data = request.json
        # Reading max_sensors and sensor_range directly from the old frontend
        params = {
...
#     # (-) Penalty for the specific cost of the sensor used
#     for i in range(num_locations):
#         for t, sensor in enumerate(sensor_types):
#             total_obj.append(x[(i, t)] * -int(sensor['cost']))
...
#             'height': float(data.get('height', 50)),
#             'priority_areas': data.get('priority_areas', []),
#             'sensor_types': data.get('sensor_types', []), # NEW
#             'density': float(data.get('density', 2.0)),
#             'w_coverage': 1000, 
#             'w_overlap': 50, 
#         }
